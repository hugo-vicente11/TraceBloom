...
[Quickstart](#quickstart-from-clone-to-a-visible-trace) · [Run the demo locally](#the-demo) · [Self-host guide](docs/self-hosting.md) · [Roadmap](#roadmap)

<!--
  HERO GIF, the first thing visitors see. Capture: `pnpm seed:agent:live`,
  open the printed URL immediately, then `pnpm eval:seed && pnpm eval:run`
  partway through so the eval chip pops in on camera. Save as
  docs/live-trace-viewer.gif (the Live tracing section reuses the same file).
  ~1400px capture, loop, <8 MB.
-->
<p align="center">
...
> This builds on the demo + landing page (M6), live tracing (M5), the
...
> real Vercel AI SDK agent. See [the demo](#the-demo) and the
> [roadmap](#roadmap).

## The demo

Two commands bring up the full stack with a curated demo project, so you can
click through the real dashboard on real telemetry rather than a mock UI:

```bash
./infra/prod/deploy.sh up      # whole stack behind Caddy, http://localhost
./infra/prod/deploy.sh seed    # a week of researcher-agent traffic
```

That drops you into a project with:
...
The same stack is built to be exposed publicly: in demo mode the path is
**provably read-only** (the dashboard holds SELECT-only database credentials,
only the reverse proxy is network-exposed, and every mutating action refuses),
and the sandbox is rate-limited, concurrency-capped and daily-budgeted
(DECISIONS.md D24–D27, all asserted in CI). Point it at a domain and Caddy
provisions HTTPS on its own: [docs/self-hosting.md](docs/self-hosting.md).
